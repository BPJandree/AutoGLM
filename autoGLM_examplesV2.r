...
# 												       #
# Example code for package autoGLM  			  					       #
# @Bo Andree											       #
# @ b.p.j.andree@vu.nl 										       #
...
# one of the useful commands that "generalizeToSpecific" calls upon is "guessStartVal".
...
# However, with large datasets the method seems to have problems with the covariance matrix.

gmmData =bestXlogit[,c(3,7,9:12)]
# I have implemented a GMM logit approach. the "guess" function works with gmm:
gmminit <- guessStartVal(Y=trainY, X=gmmData, model ="logit") 


# GMM estimation run very heavy compared to the IWLS estimator in the glm logit.
# I have mostly implemented the GMM approach to be thorough.
...
gmmlogit <- easygmmlogit(Y=trainY, X=gmmData, start =gmminit, maximizer ="nlminb")


# compare the logit with the model fitted using "generalizeToSpecific"
# predict the conditional probabilities
Pgmm <- logistic(c(coef(gmmlogit)), cbind(1,gmmData))
# obtain the predicted occurence
bingmm = Pgmm
bingmm[bingmm>0.5]<-1
bingmm[bingmm<0.5]<-0
